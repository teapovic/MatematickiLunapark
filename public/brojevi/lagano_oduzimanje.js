var game = new Phaser.Game(800, 600, Phaser.AUTO, 'laganoOduzimanje', {
    preload: preload,
    create: create,
    update: update
});
...
function generirajBrojeve() {
    brojevi[0] = game.rnd.integerInRange(1, 6);

    ukupnoElemenata = 0;
    var p;
    for (var i = 1; i < 4; i++) {
        do {
            p = game.rnd.integerInRange(brojevi[0] + 1, 10);
        } while (brojevi.indexOf(p) != -1);
        brojevi[i] = p;
        ukupnoElemenata += p - brojevi[0];
    }
}
